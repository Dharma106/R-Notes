...
cost_component_graph <- function(data, year){
  ...
  if((is.null(which(col_names == "Inputs"))))
    stop("Input Components Column is missing")
  # To check whether user has provided the arguments or not
  if(missing(year)){
    year <- readline("Enter crop year in the format 2001 for 2001-02: ")
  }
  ...
  if(all(is.na(req_crop_yr_var)== TRUE))
    stop("Mentioned crop year variable is missing")
